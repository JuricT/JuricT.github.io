...
// Скопировать себе в script.js data.json, сделать из него обычный javascript-массив
// (распарсить). С помощью lodash получить несколько массивов:
  var skills = [],
      name = [],
      friends = [];
...
//    так же они должны быть отсортированы по алфавиту;
  _.each(data, function(item) {
    _.each(item.skills, function (i) {
      if (!_.includes(skills, i)) {
        skills.push(i);
      }
    });
  });
...
//    друзей (friends);
  var sortDataName = _.sortBy(data, function (item) {
    return item.friends.length;
  });
  name = _.map(sortDataName, 'name');
  console.log(name);

  // 3. Массив всех друзей всех пользователей, не должно быть повторяющихся людей
  _.each(data, function(item) {
    _.each(item.friends, function (i) {
      var friendsName = i.name;
      if (!_.includes(friends, friendsName)) {
        friends.push(friendsName);
      }
    });
  });
  skills = _.sortBy(friends);
  console.log(friends);
